...
try:
    ...
    if not found:
        new_row = [
            'x',                                     # 0: WorkedOn
            "LC0073",                                # 1: LeetCode Number
            "73",                                    # 2: Problem Number
            "Set Matrix Zeroes",                     # 3: File Name
        ...
        ws.append(new_row)
        updated = True
        print("Added Problem 73 to xlsx")

    if updated and found:
        wb.save(file_path)
        print("Updated xlsx file with Difficulty=2, Concept 1=Array, Concept 2=Hash Table, Concept 3=Matrix for Problem 73")
    elif updated and not found:
        wb.save(file_path)
    else:
        print("xlsx file already has correct data for Problem 73")
